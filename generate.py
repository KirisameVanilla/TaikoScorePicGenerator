...
def generate_image():
    try:
        ...
        background = draw_text(text1, background, 120, (250, 150))
        background = draw_text(text2, background, 120, (250, 300))
...
if __name__ == '__main__':
    ...
    # 输入文本1
    tk.Label(root, text="曲名(日文)").grid(row=0, column=0, padx=10, pady=5)
    entry_text1 = tk.Entry(root, width=30)
    entry_text1.grid(row=0, column=1, padx=10, pady=5)

    # 输入文本2
    tk.Label(root, text="分数").grid(row=1, column=0, padx=10, pady=5)
    entry_text2 = tk.Entry(root, width=30)
    entry_text2.grid(row=1, column=1, padx=10, pady=5)

    # 输入分数类型
    tk.Label(root, text="分数类型").grid(row=2, column=0, padx=10, pady=5)
    combo_score_type = Combobox(root, values=["粉雅", "紫雅", "极"], state="readonly", width=28)
    combo_score_type.set("粉雅")  # 默认值
    combo_score_type.grid(row=2, column=1, padx=10, pady=5)

    # 难度
    tk.Label(root, text="难度").grid(row=3, column=0, padx=10, pady=5)
    combo_difficulty = Combobox(root, values=["鬼", "里鬼"], state="readonly", width=28)
    combo_difficulty.set("鬼")  # 默认值
    combo_difficulty.grid(row=3, column=1, padx=10, pady=5)

    # 皇冠
    tk.Label(root, text="皇冠").grid(row=4, column=0, padx=10, pady=5)
    combo_crown = Combobox(root, values=["通关", "全连"], state="readonly", width=28)
    combo_crown.set("通关")  # 默认值
    combo_crown.grid(row=4, column=1, padx=10, pady=5)

    # 生成图片按钮
    btn_generate = tk.Button(root, text="生成图片", command=generate_image)
    btn_generate.grid(row=5, column=0, columnspan=2, pady=20)
